...
// Internal helpers exposed to device via declare target
...
static inline unsigned long long rng_next_u64(unsigned long long& s0, unsigned long long& s1)
{
...
int rng_uniform_int(unsigned long long& s0, unsigned long long& s1, int lo, int hi)
{
    unsigned long long r = rng_next_u64(s0, s1);
    int width = hi - lo + 1;
    return lo + (int)(r % (unsigned long long)width);
}
...
{
    #pragma omp target teams distribute parallel for map(tofrom: s0[0:count], s1[0:count])
    for (int i = 0; i < count; ++i) {
        unsigned long long x = seed ^ (0x9E3779B97F4A7C15ULL * (unsigned long long)(i + 1));
        unsigned long long a = splitmix64_next(x);
        unsigned long long b = splitmix64_next(x);
        if (a == 0 && b == 0) b = 1ULL; // avoid zero state
        s0[i] = a; s1[i] = b;
    }
}
